...
def get_ablation_settings(include_disjoint: bool) -> list[dict[str, Any]]:
    settings = [
        {"name": "domain_only", "use_domain": True, "use_range": False, "use_disjoint": False, "binary_like": False},
        {"name": "range_only", "use_domain": False, "use_range": True, "use_disjoint": False, "binary_like": False},
        {"name": "domain_range", "use_domain": True, "use_range": True, "use_disjoint": False, "binary_like": False},
        {"name": "binary_domain_range", "use_domain": True, "use_range": True, "use_disjoint": False, "binary_like": True},
    ]

    if include_disjoint:
        settings.extend([
            {"name": "disjoint_only", "use_domain": False, "use_range": False, "use_disjoint": True, "binary_like": False},
            {"name": "domain_disjoint", "use_domain": True, "use_range": False, "use_disjoint": True, "binary_like": False},
            {"name": "range_disjoint", "use_domain": False, "use_range": True, "use_disjoint": True, "binary_like": False},
            {"name": "full", "use_domain": True, "use_range": True, "use_disjoint": True, "binary_like": False},
            {"name": "binary_full", "use_domain": True, "use_range": True, "use_disjoint": True, "binary_like": True},
        ])
    else:
        settings.append({"name": "full", "use_domain": True, "use_range": True, "use_disjoint": False, "binary_like": False})
...
def compute_progress_payload(
    *,
    ...
    for name, agg in aggs.items():
        ...
        running[f"{name}_viol_at_k"] = (agg["viol_sum"] / fn) if fn > 0 else None
        running[f"{name}_adm_at_k"] = (agg["adm_sum"] / fn) if fn > 0 else None
...
def run_exp06(args: argparse.Namespace) -> None:
    ...
    include_disjoint = bool(args.include_disjoint and ontology.has_disjoint_pairs)
    logger.log(
        ...
        f"include_disjoint={include_disjoint}"
    )
    ...
    logger.log("[STEP 5/7] Building query list and ablation plan")
    ...
    settings = get_ablation_settings(include_disjoint=include_disjoint)
    ...
    logger.log("[STEP 6/7] Running exact ablation benchmarks with collapse-oriented reporting")
    ...
    for batch_start in pbar:
        ...
        for row_idx, q in enumerate(batch_queries):
            ...
            for setting in settings:
                ...
                rel_counter = rel_stats[name][q.relation_name]
                ...
                if feasible:
                    ...
                    collapse_exact_match = compute_topk_exact_match(adfirst_topk, reranked_topk)
                    collapse_set_match = compute_topk_set_match(adfirst_topk, reranked_topk)
                    collapse_jaccard = compute_topk_set_jaccard(adfirst_topk, reranked_topk)

                    aggs[name]["num_feasible"] += 1
                    aggs[name]["viol_sum"] += optk_metrics["viol_at_k"]
                    aggs[name]["adm_sum"] += optk_metrics["adm_at_k"]
                    aggs[name]["cov_sum"] += optk_metrics["cov_at_k"]
                    aggs[name]["pres_sum"] += optk_metrics["pres_at_k"]
                    aggs[name]["shift_sum"] += optk_metrics["shift_at_k"]
                    ...
                    aggs[name]["collapse_exact_match_sum"] += collapse_exact_match
                    aggs[name]["collapse_set_match_sum"] += collapse_set_match
                    aggs[name]["collapse_jaccard_sum"] += collapse_jaccard

                    aggs[name]["lambda_values"].append(lambda_star)

                    rel_counter["num_feasible"] += 1
                    rel_counter["viol_sum"] += optk_metrics["viol_at_k"]
                    rel_counter["adm_sum"] += optk_metrics["adm_at_k"]
                    rel_counter["cov_sum"] += optk_metrics["cov_at_k"]
                    rel_counter["pres_sum"] += optk_metrics["pres_at_k"]
                    rel_counter["shift_sum"] += optk_metrics["shift_at_k"]
    ...
    full_name = "full"
    full_feasible_rate = None
    if full_name in aggs:
        full_n = aggs[full_name]["num_queries"]
        full_feasible_rate = (aggs[full_name]["num_feasible"] / full_n) if full_n > 0 else None
    ...
    with summary_csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for setting in settings:
            ...
            delta_feasible = (
                (feasible_rate - full_feasible_rate)
                if feasible_rate is not None and full_feasible_rate is not None
                else None
            )
            ...
            writer.writerow([
                ...
                (agg["viol_sum"] / fn) if fn > 0 else None,
                (agg["adm_sum"] / fn) if fn > 0 else None,
                (agg["cov_sum"] / fn) if fn > 0 else None,
                (agg["pres_sum"] / fn) if fn > 0 else None,
                (agg["shift_sum"] / fn) if fn > 0 else None,
        ...
        for setting in settings:
            ...
            for rel, c in sorted(rel_stats[name].items()):
                ...
                writer.writerow([
                    ...
                    c["queries"],
                    c["num_feasible"],
                    (c["num_feasible"] / qn) if qn > 0 else None,

                    (c["viol_sum"] / fn) if fn > 0 else None,
                    (c["adm_sum"] / fn) if fn > 0 else None,
                    (c["cov_sum"] / fn) if fn > 0 else None,
                    (c["pres_sum"] / fn) if fn > 0 else None,
                    (c["shift_sum"] / fn) if fn > 0 else None,
    ...
    summary = {
        ...
        "experiment_name": "ablations_exact",
        ...
        "semantic_policy": {
            "check_policy": args.check_policy,
            "unknown_penalty": args.unknown_penalty,
            "include_disjoint_requested": args.include_disjoint,
            "include_disjoint_used": include_disjoint,
            "note": (
                "Each ablation defines its own active constraint family and binary-like or graded energy. "
                "AdmissibleFirst is reported with fill-to-K to keep collapse comparisons comparable to OptK."
            ),
        },
    ...
    for setting in settings:
        ...
        summary["settings"][name] = {
            ...
            "optk": {
                "viol_at_k": (agg["viol_sum"] / fn) if fn > 0 else None,
                "adm_at_k": (agg["adm_sum"] / fn) if fn > 0 else None,
                "cov_at_k": (agg["cov_sum"] / fn) if fn > 0 else None,
                "pres_at_k": (agg["pres_sum"] / fn) if fn > 0 else None,
                "shift_at_k": (agg["shift_sum"] / fn) if fn > 0 else None,
            },
            ...
            "delta_feasible_vs_full": delta_feasible,
        }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EXP-06: Exact ablation study across constraint families with collapse-oriented reporting."
    )
