...
def write_query_csv_header(path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "scoring_ms",
            "semantic_ms",
            "lambda_ms_feasible_only",
            "rerank_ms_feasible_only",
            "post_scoring_ms",
            "amortized_total_ms",
        ])
    ...
    *,
    ...
    return {
        ...
        "running_metrics": {
            ...
            "used_optk_rate": (agg["num_feasible"] / n) if n > 0 else None,
            "mean_scoring_ms": (agg["scoring_time_sum_ms"] / n) if n > 0 else None,
            "mean_semantic_ms": (agg["semantic_time_sum_ms"] / n) if n > 0 else None,
            "mean_post_scoring_ms": (agg["post_scoring_time_sum_ms"] / n) if n > 0 else None,
            "mean_amortized_total_ms": (agg["amortized_total_time_sum_ms"] / n) if n > 0 else None,
            "mean_lambda_ms_feasible_only": (agg["lambda_time_sum_ms_feasible_only"] / fn) if fn > 0 else None,
            "mean_rerank_ms_feasible_only": (agg["rerank_time_sum_ms_feasible_only"] / fn) if fn > 0 else None,
        },
    }
...
def run_exp04(args: argparse.Namespace) -> None:
    ...
    agg = {
        ...
        "scoring_times_ms": [],
        "semantic_times_ms": [],
        "lambda_times_ms_feasible_only": [],
        "rerank_times_ms_feasible_only": [],
        "post_scoring_times_ms": [],
        "amortized_total_times_ms": [],

        "scoring_time_sum_ms": 0.0,
        "semantic_time_sum_ms": 0.0,
        "lambda_time_sum_ms_feasible_only": 0.0,
        "rerank_time_sum_ms_feasible_only": 0.0,
        "post_scoring_time_sum_ms": 0.0,
        "amortized_total_time_sum_ms": 0.0,
    }
    ...
    logger.log("[STEP 6/7] Measuring exact practicality / runtime")
    ...
    for batch_start in pbar:
        ...
        scoring_batch_ms = (perf_now() - t_score) * 1000.0
        per_query_scoring_ms = scoring_batch_ms / max(1, len(batch_queries))
        ...
        for row_idx, q in enumerate(batch_queries):
            ...
            semantic_ms = (perf_now() - t_sem) * 1000.0
            ...
            lambda_ms_feasible_only = (perf_now() - t_lambda) * 1000.0

            rerank_ms_feasible_only = 0.0
            if feasible:
                t_rerank = perf_now()
                _ = stable_rerank_with_lambda(
                    cand_ids=cand_ids,
                    base_scores=cand_scores,
                    energies=energies,
                    lam=lambda_star,
                )[:args.top_k]
                rerank_ms_feasible_only = (perf_now() - t_rerank) * 1000.0

            post_scoring_ms = (perf_now() - query_t0) * 1000.0
            amortized_total_ms = per_query_scoring_ms + post_scoring_ms

            agg["num_queries"] += 1
            agg["num_feasible"] += int(feasible)

            agg["scoring_times_ms"].append(per_query_scoring_ms)
            agg["semantic_times_ms"].append(semantic_ms)
            agg["post_scoring_times_ms"].append(post_scoring_ms)
            agg["amortized_total_times_ms"].append(amortized_total_ms)

            agg["scoring_time_sum_ms"] += per_query_scoring_ms
            agg["semantic_time_sum_ms"] += semantic_ms
            agg["post_scoring_time_sum_ms"] += post_scoring_ms
            agg["amortized_total_time_sum_ms"] += amortized_total_ms

            if feasible:
                agg["lambda_values"].append(lambda_star)
                agg["lambda_times_ms_feasible_only"].append(lambda_ms_feasible_only)
                agg["rerank_times_ms_feasible_only"].append(rerank_ms_feasible_only)
                agg["lambda_time_sum_ms_feasible_only"] += lambda_ms_feasible_only
                agg["rerank_time_sum_ms_feasible_only"] += rerank_ms_feasible_only

            rel_counter = rel_stats[q.relation_name]
            rel_counter["queries"] += 1
            rel_counter["num_feasible"] += int(feasible)

            rel_counter["scoring_sum_ms"] += per_query_scoring_ms
            rel_counter["semantic_sum_ms"] += semantic_ms
            rel_counter["post_scoring_sum_ms"] += post_scoring_ms
            rel_counter["amortized_total_sum_ms"] += amortized_total_ms

            if feasible:
                rel_counter["lambda_sum_ms_feasible_only"] += lambda_ms_feasible_only
                rel_counter["rerank_sum_ms_feasible_only"] += rerank_ms_feasible_only
                rel_counter["lambda_value_sum"] += lambda_star
                rel_counter["lambda_value_count"] += 1
            ...
            query_rows_buffer.append([
                ...
                tau_k,
                effective_top_m,
                per_query_scoring_ms,
                semantic_ms,
                lambda_ms_feasible_only if feasible else None,
                rerank_ms_feasible_only if feasible else None,
                post_scoring_ms,
                amortized_total_ms,
            ])
            processed += 1
        ...
        feasible_rate = progress_payload["running_metrics"]["feasible_rate"]
        mean_amortized_total_ms = progress_payload["running_metrics"]["mean_amortized_total_ms"]

        pbar.set_postfix({
            "q": f"{processed}/{total_queries}",
            "feas": f"{feasible_rate:.4f}" if feasible_rate is not None else "nan",
            "amort_ms": f"{mean_amortized_total_ms:.2f}" if mean_amortized_total_ms is not None else "nan",
        })
        ...
        if (batch_number % args.log_every) == 0 or batch_end == total_queries:
            logger.log(
                ...
                f"feasible_rate={feasible_rate if feasible_rate is not None else 'NA'} "
                f"mean_amortized_total_ms={mean_amortized_total_ms if mean_amortized_total_ms is not None else 'NA'}"
            )
    ...
    with relation_csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "mean_scoring_ms",
            "mean_semantic_ms",
            "mean_post_scoring_ms",
            "mean_amortized_total_ms",
            "mean_lambda_ms_feasible_only",
            "mean_rerank_ms_feasible_only",
            "mean_lambda_value",
        ])
        ...
        for rel, c in sorted(rel_stats.items()):
            ...
            writer.writerow([
                ...
                (fn / qn) if qn > 0 else None,
                (c["scoring_sum_ms"] / qn) if qn > 0 else None,
                (c["semantic_sum_ms"] / qn) if qn > 0 else None,
                (c["post_scoring_sum_ms"] / qn) if qn > 0 else None,
                (c["amortized_total_sum_ms"] / qn) if qn > 0 else None,
                (c["lambda_sum_ms_feasible_only"] / fn) if fn > 0 else None,
                (c["rerank_sum_ms_feasible_only"] / fn) if fn > 0 else None,
                (c["lambda_value_sum"] / c["lambda_value_count"]) if c["lambda_value_count"] > 0 else None,
            ])
    ...
    summary = {
        ...
        "experiment_name": "practicality_exact",
        ...
        "semantic_policy": {
            ...
            "unknown_penalty": args.unknown_penalty,
            "note": (
                "EXP-04 measures practicality of exact OptK. "
                "Lambda and rerank timings are reported on feasible queries only."
            ),
        },
        ...
        "queries": {
            ...
            "used_optk_rate": (fn / n) if n > 0 else None,
            "throughput_queries_per_sec": (n / total_elapsed) if total_elapsed > 0 else None,
        },

        "runtime_ms": {
            "mean_scoring_ms": (agg["scoring_time_sum_ms"] / n) if n > 0 else None,
            "median_scoring_ms": statistics.median(agg["scoring_times_ms"]) if agg["scoring_times_ms"] else None,
            "p95_scoring_ms": percentile(agg["scoring_times_ms"], 95),

            "mean_semantic_ms": (agg["semantic_time_sum_ms"] / n) if n > 0 else None,
            "median_semantic_ms": statistics.median(agg["semantic_times_ms"]) if agg["semantic_times_ms"] else None,
            "p95_semantic_ms": percentile(agg["semantic_times_ms"], 95),

            "mean_post_scoring_ms": (agg["post_scoring_time_sum_ms"] / n) if n > 0 else None,
            "median_post_scoring_ms": statistics.median(agg["post_scoring_times_ms"]) if agg["post_scoring_times_ms"] else None,
            "p95_post_scoring_ms": percentile(agg["post_scoring_times_ms"], 95),

            "mean_amortized_total_ms": (agg["amortized_total_time_sum_ms"] / n) if n > 0 else None,
            "median_amortized_total_ms": statistics.median(agg["amortized_total_times_ms"]) if agg["amortized_total_times_ms"] else None,
            "p95_amortized_total_ms": percentile(agg["amortized_total_times_ms"], 95),

            "mean_lambda_ms_feasible_only": (agg["lambda_time_sum_ms_feasible_only"] / fn) if fn > 0 else None,
            "median_lambda_ms_feasible_only": statistics.median(agg["lambda_times_ms_feasible_only"]) if agg["lambda_times_ms_feasible_only"] else None,
            "p95_lambda_ms_feasible_only": percentile(agg["lambda_times_ms_feasible_only"], 95),

            "mean_rerank_ms_feasible_only": (agg["rerank_time_sum_ms_feasible_only"] / fn) if fn > 0 else None,
            "median_rerank_ms_feasible_only": statistics.median(agg["rerank_times_ms_feasible_only"]) if agg["rerank_times_ms_feasible_only"] else None,
            "p95_rerank_ms_feasible_only": percentile(agg["rerank_times_ms_feasible_only"], 95),
        },

        "lambda_stats": {
            "num_feasible": fn,
            "used_optk_rate": (fn / n) if n > 0 else None,
            "min_lambda": min(agg["lambda_values"]) if agg["lambda_values"] else None,
    ...
    logger.log(
        f"[DONE] feasible_rate={summary['queries']['feasible_rate'] if summary['queries']['feasible_rate'] is not None else 'NA'} "
        f"mean_amortized_total_ms={summary['runtime_ms']['mean_amortized_total_ms'] if summary['runtime_ms']['mean_amortized_total_ms'] is not None else 'NA'} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EXP-04: Practicality diagnostics for exact OptK over frozen KGC rankings."
    )
