...
def normalize_quotas(quotas: list[int], top_k: int) -> list[int]:
    out = sorted(set(int(q) for q in quotas))
    if not out:
        out = list(range(1, top_k + 1))
    for q in out:
        if q < 1:
    ...
    for row_index, row in enumerate(mapped_triples.tolist()):
        ...
        if mode in ("tail", "all"):
            ...
            if allowed_query_ids is None or qid in allowed_query_ids:
                queries.append(QueryItem(qid, row_index, "tail", h, r, t, rel_name))

        if mode in ("head", "all"):
            qid = f"{split_name}_head_{row_index:09d}"
            if allowed_query_ids is None or qid in allowed_query_ids:
                queries.append(QueryItem(qid, row_index, "head", h, r, t, rel_name))
...
def write_query_csv_header(path: Path, quotas: list[int]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        header = [
            ...
            "effective_top_m",
            "is_blind",
            "base_violating_in_topk",
            "base_admissible_in_topm",
        ]
        for q in quotas:
            header.extend([
                f"q{q}_feasible",
...
def compute_progress_payload(
    *,
    ...
    running = {
        "blind_subset_rate_so_far": blind_queries_seen / processed_queries if processed_queries else None,
    }

    for q, agg in quota_aggs.items():
        n_blind = agg["num_blind_queries"]
        n_feas = agg["num_feasible"]
        running[f"q{q}_feasible_rate_among_blind"] = (n_feas / n_blind) if n_blind > 0 else None
        running[f"q{q}_viol_at_k"] = (agg["viol_sum"] / n_feas) if n_feas > 0 else None
        running[f"q{q}_adm_at_k"] = (agg["adm_sum"] / n_feas) if n_feas > 0 else None
        running[f"q{q}_pres_at_k"] = (agg["pres_sum"] / n_feas) if n_feas > 0 else None
        running[f"q{q}_shift_at_k"] = (agg["shift_sum"] / n_feas) if n_feas > 0 else None
...
def save_frontier_plots(
    *,
    ...
    for q in quotas:
        ...
        feasible_rate = (n_feas / n_blind) if n_blind > 0 else np.nan
        feas.append(feasible_rate)

        if n_feas > 0:
            viol.append(agg["viol_sum"] / n_feas)
            adm.append(agg["adm_sum"] / n_feas)
            pres.append(agg["pres_sum"] / n_feas)
            shift.append(agg["shift_sum"] / n_feas)
            med_lambda.append(statistics.median(agg["lambda_values"]) if agg["lambda_values"] else np.nan)
            p95_lambda.append(percentile(agg["lambda_values"], 95) if agg["lambda_values"] else np.nan)
        else:
            viol.append(np.nan)
            adm.append(np.nan)
            pres.append(np.nan)
    ...
    ax.plot(xs, feas, marker="o")
    ax.set_xlabel("Quota q")
    ax.set_ylabel("Feasible rate among blind")
    ax.set_title(f"{dataset_name} | Quota feasibility frontier")
    ...
    ax.set_title(f"{dataset_name} | Quota semantics frontier")
    ax.grid(True, alpha=0.3)
    ax.legend()
    ...
    ax.set_title(f"{dataset_name} | Quota perturbation frontier")
    ax.grid(True, alpha=0.3)
    ax.legend()
    ...
    ax.set_title(f"{dataset_name} | Quota intervention frontier")
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def run_exp07(args: argparse.Namespace) -> None:
    ...
    logger.log("[STEP 6/7] Scoring and evaluating quota control on blind subset")
    ...
    for batch_start in range(0, total_queries, args.query_batch_size):
        ...
        for row_idx, q in enumerate(batch_queries):
            ...
            for rank_idx, cand_id in enumerate(cand_ids):
                ...
                if rank_idx < args.top_k and sem.checkable and sem.violated:
                    violating_in_topk = True

            is_blind = violating_in_topk and (admissible_in_topm > 0)
            ...
            if is_blind:
                ...
                for quota in quotas:
                    quota_aggs[quota]["num_blind_queries"] += 1
                    ...
                    if feasible:
                        ...
                        reranked_sem = [
                            (checkable_by_id[cid], admissible_by_id[cid], violated_by_id[cid])
                            for cid in reranked_topk
                        ]

                        checkable_count = sum(1 for c, _, _ in reranked_sem if c)
                        violating_count = sum(1 for c, _, v in reranked_sem if c and v)
                        admissible_count = sum(1 for _, a, _ in reranked_sem if a)

                        viol_at_k = (violating_count / checkable_count) if checkable_count > 0 else 0.0
                        adm_at_k = admissible_count / float(args.top_k)
                ...
                for quota in quotas:
                    row.extend([0, None, None, None, None, None, None])
        ...
        progress_payload = compute_progress_payload(
            ...
            total_queries=total_queries,
            blind_queries_seen=blind_queries_total,
            effective_top_m=effective_top_m,
        ...
        if (batch_number % args.log_every) == 0 or batch_end == total_queries:
            msg = (
                ...
                f"blind_so_far={progress_payload['running_metrics']['blind_subset_rate_so_far'] if progress_payload['running_metrics']['blind_subset_rate_so_far'] is not None else 'NA'}"
            )
            if quotas:
                qmax = quotas[-1]
                qmax_val = progress_payload["running_metrics"].get(f"q{qmax}_feasible_rate_among_blind")
                msg += f" q{qmax}_feas={qmax_val if qmax_val is not None else 'NA'}"
            logger.log(msg)
    ...
    logger.log("[STEP 7/7] Building frontier tables, plots, and final summary")
    ...
    with frontier_csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for quota in quotas:
            ...
            med_lambda = statistics.median(agg["lambda_values"]) if agg["lambda_values"] else None
            p95_lambda = percentile(agg["lambda_values"], 95)

            writer.writerow([
                args.dataset_name or args.processed_dir.name,
                quota,
                agg["num_blind_queries"],
                agg["num_feasible"],
                (n_feas / n_blind) if n_blind > 0 else None,
                (agg["viol_sum"] / n_feas) if n_feas > 0 else None,
                (agg["adm_sum"] / n_feas) if n_feas > 0 else None,
                (agg["pres_sum"] / n_feas) if n_feas > 0 else None,
                (agg["shift_sum"] / n_feas) if n_feas > 0 else None,
                mean_lambda,
                med_lambda,
                p95_lambda,
            ])
    ...
    summary = {
        ...
        "experiment_name": "quota_blind_subset_bench",
        ...
        "semantic_policy": {
            ...
            "unknown_penalty": args.unknown_penalty,
            "note": (
                "EXP-07 evaluates OptQ only on the blind subset. "
                "Quota feasibility is defined relative to the candidate window top-M."
            ),
        },
        ...
        "queries": {
            "total_queries": total_queries,
            "blind_queries_count": blind_queries_total,
            "blind_subset_rate": (blind_queries_total / total_queries) if total_queries else None,
        },
    ...
    for quota in quotas:
        ...
        summary["quotas"][str(quota)] = {
            "num_blind_queries": agg["num_blind_queries"],
            "num_feasible": agg["num_feasible"],
            "feasible_rate_among_blind": (n_feas / n_blind) if n_blind > 0 else None,
            "viol_at_k": (agg["viol_sum"] / n_feas) if n_feas > 0 else None,
            "adm_at_k": (agg["adm_sum"] / n_feas) if n_feas > 0 else None,
            "pres_at_k": (agg["pres_sum"] / n_feas) if n_feas > 0 else None,
            "shift_at_k": (agg["shift_sum"] / n_feas) if n_feas > 0 else None,
            "lambda_stats": {
                "mean_lambda": safe_mean(agg["lambda_values"]),
    ...
    logger.log(
        f"[DONE] blind_subset_rate={summary['queries']['blind_subset_rate'] if summary['queries']['blind_subset_rate'] is not None else 'NA'} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EXP-07: Quota blind-subset benchmark over frozen KGC rankings."
    )
...
def main() -> None:
    ...
    if args.quotas is None or len(args.quotas) == 0:
        args.quotas = list(range(1, args.top_k + 1))
