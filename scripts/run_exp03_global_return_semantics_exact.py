...
def write_query_csv_header(path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "collapse_adfirst_optk_exact_match",
            "collapse_adfirst_optk_set_match",
            "collapse_adfirst_optk_jaccard",
        ])
    ...
    *,
    ...
    return {
        ...
        "running_metrics": {
            ...
            "base_viol_at_k": (agg["base_viol_sum"] / n) if n > 0 else None,
            "base_adm_at_k": (agg["base_adm_sum"] / n) if n > 0 else None,

            "hardval_viol_at_k": (agg["hardval_viol_sum"] / n_feas) if n_feas > 0 else None,
            "hardval_adm_at_k": (agg["hardval_adm_sum"] / n_feas) if n_feas > 0 else None,

            "adfirst_viol_at_k": (agg["adfirst_viol_sum"] / n_feas) if n_feas > 0 else None,
            "adfirst_adm_at_k": (agg["adfirst_adm_sum"] / n_feas) if n_feas > 0 else None,

            "glincal_viol_at_k": (agg["glincal_viol_sum"] / n_feas) if n_feas > 0 else None,
            "glincal_adm_at_k": (agg["glincal_adm_sum"] / n_feas) if n_feas > 0 else None,

            "optk_viol_at_k": (agg["optk_viol_sum"] / n_feas) if n_feas > 0 else None,
            "optk_adm_at_k": (agg["optk_adm_sum"] / n_feas) if n_feas > 0 else None,

            "collapse_exact_match": (agg["collapse_exact_match_sum"] / n_feas) if n_feas > 0 else None,
            "collapse_set_match": (agg["collapse_set_match_sum"] / n_feas) if n_feas > 0 else None,
            "collapse_jaccard": (agg["collapse_jaccard_sum"] / n_feas) if n_feas > 0 else None,
        },
    }
...
def run_exp03(args: argparse.Namespace) -> None:
    ...
    logger.log("[STEP 6/7] Scoring and evaluating global exact semantics + baselines")
    ...
    for batch_start in pbar:
        ...
        for row_idx, q in enumerate(batch_queries):
            ...
            collapse_exact_match = None
            collapse_set_match = None
            ...
            if feasible:
                ...
                collapse_exact_match = compute_topk_exact_match(adfirst_topk, optk_topk)
                collapse_set_match = compute_topk_set_match(adfirst_topk, optk_topk)
                collapse_jaccard = compute_topk_set_jaccard(adfirst_topk, optk_topk)

                agg["collapse_exact_match_sum"] += collapse_exact_match
                agg["collapse_set_match_sum"] += collapse_set_match
                agg["collapse_jaccard_sum"] += collapse_jaccard

                rel_counter["collapse_exact_match_sum"] += collapse_exact_match
                rel_counter["collapse_set_match_sum"] += collapse_set_match
        ...
        base_viol = progress_payload["running_metrics"]["base_viol_at_k"]
        optk_viol = progress_payload["running_metrics"]["optk_viol_at_k"]
        collapse_set_match = progress_payload["running_metrics"]["collapse_set_match"]

        pbar.set_postfix({
            "q": f"{processed}/{total_queries}",
            "feas": f"{feasible_rate:.4f}" if feasible_rate is not None else "nan",
            "baseV": f"{base_viol:.4f}" if base_viol is not None else "nan",
            "optkV": f"{optk_viol:.4f}" if optk_viol is not None else "nan",
            "setMatch": f"{collapse_set_match:.4f}" if collapse_set_match is not None else "nan",
        })
        ...
        if (batch_number % args.log_every) == 0 or batch_end == total_queries:
            logger.log(
                ...
                f"base_viol={base_viol if base_viol is not None else 'NA'} "
                f"optk_viol={optk_viol if optk_viol is not None else 'NA'} "
                f"collapse_set_match={collapse_set_match if collapse_set_match is not None else 'NA'}"
            )
    ...
    with relation_csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for rel, c in sorted(rel_stats.items()):
            ...
            writer.writerow([
                ...
                (c["adfirst_viol_sum"] / fn) if fn > 0 else None,
                (c["adfirst_adm_sum"] / fn) if fn > 0 else None,
                (c["adfirst_cov_sum"] / fn) if fn > 0 else None,
                (c["adfirst_pres_sum"] / fn) if fn > 0 else None,
                (c["adfirst_shift_sum"] / fn) if fn > 0 else None,

                (c["glincal_viol_sum"] / fn) if fn > 0 else None,
                (c["glincal_adm_sum"] / fn) if fn > 0 else None,
                (c["glincal_cov_sum"] / fn) if fn > 0 else None,
                (c["glincal_pres_sum"] / fn) if fn > 0 else None,
                (c["glincal_shift_sum"] / fn) if fn > 0 else None,

                (c["optk_viol_sum"] / fn) if fn > 0 else None,
                (c["optk_adm_sum"] / fn) if fn > 0 else None,
                (c["optk_cov_sum"] / fn) if fn > 0 else None,
                (c["optk_pres_sum"] / fn) if fn > 0 else None,
                (c["optk_shift_sum"] / fn) if fn > 0 else None,

                (c["collapse_exact_match_sum"] / fn) if fn > 0 else None,
    ...
    summary = {
        ...
        "experiment_name": "global_return_semantics_exact",
        ...
        "semantic_policy": {
            ...
            "unknown_penalty": args.unknown_penalty,
            "note": (
                "Exact feasibility is defined by zero-energy admissible candidates only; "
        ...
        "collapse_definition": {
            ...
            "exact_match_rate": "fraction of feasible queries where ordered top-K lists are identical",
            "set_match_rate": "fraction of feasible queries where top-K sets are identical",
            "mean_jaccard": "mean top-K set Jaccard similarity across feasible queries",
        },
        ...
        "queries": {
            ...
            "median_target_rank": median_target_rank,
        },

        "base": {
            "viol_at_k": (agg["base_viol_sum"] / n) if n > 0 else None,
            "adm_at_k": (agg["base_adm_sum"] / n) if n > 0 else None,
        },
        ...
        "glincal_on_feasible": {
            "viol_at_k": (agg["glincal_viol_sum"] / fn) if fn > 0 else None,
            "adm_at_k": (agg["glincal_adm_sum"] / fn) if fn > 0 else None,
            "cov_at_k": (agg["glincal_cov_sum"] / fn) if fn > 0 else None,
            "pres_at_k": (agg["glincal_pres_sum"] / fn) if fn > 0 else None,
            "shift_at_k": (agg["glincal_shift_sum"] / fn) if fn > 0 else None,
    ...
    logger.log(
        ...
        f"base_viol={summary['base']['viol_at_k'] if summary['base']['viol_at_k'] is not None else 'NA'} "
        f"hardval_viol={summary['hardval_on_feasible']['viol_at_k'] if summary['hardval_on_feasible']['viol_at_k'] is not None else 'NA'} "
        f"adfirst_viol={summary['admissible_first_on_feasible']['viol_at_k'] if summary['admissible_first_on_feasible']['viol_at_k'] is not None else 'NA'} "
        f"glincal_viol={summary['glincal_on_feasible']['viol_at_k'] if summary['glincal_on_feasible']['viol_at_k'] is not None else 'NA'} "
        f"optk_viol={summary['optk_on_feasible']['viol_at_k'] if summary['optk_on_feasible']['viol_at_k'] is not None else 'NA'} "
        f"collapse_set_match={summary['collapse']['set_match_rate'] if summary['collapse']['set_match_rate'] is not None else 'NA'} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EXP-03: Global returned-list semantics for exact OptK over frozen KGC rankings with baselines and collapse analysis."
    )
