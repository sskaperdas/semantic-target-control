...
def evaluate_candidate_semantics(
    *,
    ...
    violated = checkable and (num_violations > 0)
    ...
    if admissible:
        ...
        ua_energy = float(beta)
    else:
        ua_energy = float(max(1, num_violations))
...
def compute_lambda_star_exact_ua(
    *,
    base_scores: list[float],
    ua_energies: list[float],
    admissible_flags: list[bool],
    k: int,
) -> tuple[bool, Optional[float], Optional[float]]:
    zero_idxs = [i for i, ok in enumerate(admissible_flags) if ok]
    if len(zero_idxs) < k:
    ...
    candidates = []
    for s, e in zip(base_scores, ua_energies):
        if e > 0.0:
...
def stable_rerank_with_lambda(
    *,
    ...
    items = []
    for idx, (cid, s, e) in enumerate(zip(cand_ids, base_scores, ua_energies)):
        calibrated = s - lam * e
...
def unknown_aware_fallback_ranking(
    *,
    cand_ids: list[int],
    base_scores: list[float],
    ...
    items = []
    for idx, (cid, s, st) in enumerate(zip(cand_ids, base_scores, statuses)):
        if st.admissible:
            bucket = 0
        elif st.unknown:
            bucket = 1
            ...
            bucket = 2
        items.append((cid, bucket, st.ua_energy, -s, idx))
    items.sort(key=lambda x: (x[1], x[2], x[3], x[4]))
    return [x[0] for x in items]
...
def compute_progress_payload(
    *,
    ...
    return {
        ...
        "running_metrics": {
            "exact_feasible_rate": (agg["num_exact_feasible"] / n) if n > 0 else None,
            "used_exact_optkua_rate": (agg["num_exact_feasible"] / n) if n > 0 else None,
            "used_fallback_rate": (agg["num_fallback"] / n) if n > 0 else None,
            "base_viol_at_k": (agg["base_viol_sum"] / n) if n > 0 else None,
            "base_unknown_at_k": (agg["base_unknown_sum"] / n) if n > 0 else None,
            "optkua_viol_at_k": (agg["optkua_viol_sum"] / n) if n > 0 else None,
            "optkua_unknown_at_k": (agg["optkua_unknown_sum"] / n) if n > 0 else None,
            "optkua_adm_at_k": (agg["optkua_adm_sum"] / n) if n > 0 else None,
        },
    }
...
def run_exp05(args: argparse.Namespace) -> None:
    ...
    agg = {
        ...
        "base_adm_sum": 0.0,
        "base_cov_sum": 0.0,
        "base_unknown_sum": 0.0,

        "optkua_viol_sum": 0.0,
        "optkua_adm_sum": 0.0,
        "optkua_cov_sum": 0.0,
        "optkua_unknown_sum": 0.0,
        "pres_sum": 0.0,
        "shift_sum": 0.0,
    }
    ...
    logger.log("[STEP 6/7] Evaluating unknown-aware global deployment policy")
    ...
    for batch_start in pbar:
        ...
        for row_idx, q in enumerate(batch_queries):
            ...
            for rank_idx, cand_id in enumerate(cand_ids):
                ...
                statuses.append(st)
                ua_energies.append(st.ua_energy)
                admissible_flags.append(st.admissible)
            ...
            base_unknown_at_k = base_unknown_topk / float(args.top_k)

            agg["num_queries"] += 1
            agg["base_viol_sum"] += base_viol_at_k
            agg["base_adm_sum"] += base_adm_at_k
            agg["base_cov_sum"] += base_cov_at_k
            agg["base_unknown_sum"] += base_unknown_at_k
            ...
            rel_counter["base_unknown_sum"] += base_unknown_at_k

            feasible, lambda_star, tau_k = compute_lambda_star_exact_ua(
                base_scores=cand_scores,
                ua_energies=ua_energies,
                admissible_flags=admissible_flags,
                k=args.top_k,
            )

            if feasible:
                agg["num_exact_feasible"] += 1
                agg["lambda_values"].append(lambda_star)
                rel_counter["num_exact_feasible"] += 1
                ...
                agg["num_fallback"] += 1
                rel_counter["num_fallback"] += 1

                reranked = unknown_aware_fallback_ranking(
                    cand_ids=cand_ids,
                    base_scores=cand_scores,
                    statuses=statuses,
                )
            ...
            optkua_viol_at_k = (out_violating / out_checkable) if out_checkable > 0 else 0.0
            optkua_adm_at_k = out_admissible / float(args.top_k)
            optkua_cov_at_k = out_checkable / float(args.top_k)
            optkua_unknown_at_k = out_unknown / float(args.top_k)
            ...
            agg["optkua_viol_sum"] += optkua_viol_at_k
            agg["optkua_adm_sum"] += optkua_adm_at_k
            agg["optkua_cov_sum"] += optkua_cov_at_k
            agg["optkua_unknown_sum"] += optkua_unknown_at_k
            agg["pres_sum"] += pres_at_k
            agg["shift_sum"] += shift_at_k

            rel_counter["optkua_viol_sum"] += optkua_viol_at_k
            rel_counter["optkua_adm_sum"] += optkua_adm_at_k
            rel_counter["optkua_cov_sum"] += optkua_cov_at_k
            rel_counter["optkua_unknown_sum"] += optkua_unknown_at_k
            rel_counter["pres_sum"] += pres_at_k
            rel_counter["shift_sum"] += shift_at_k
        ...
        exact_feasible_rate = progress_payload["running_metrics"]["exact_feasible_rate"]
        base_unknown = progress_payload["running_metrics"]["base_unknown_at_k"]
        optkua_unknown = progress_payload["running_metrics"]["optkua_unknown_at_k"]

        pbar.set_postfix({
            "q": f"{processed}/{total_queries}",
            "exact": f"{exact_feasible_rate:.4f}" if exact_feasible_rate is not None else "nan",
            "baseUnk": f"{base_unknown:.4f}" if base_unknown is not None else "nan",
            "optUnk": f"{optkua_unknown:.4f}" if optkua_unknown is not None else "nan",
        })
        ...
        if (batch_number % args.log_every) == 0 or batch_end == total_queries:
            logger.log(
                ...
                f"exact_feasible={exact_feasible_rate if exact_feasible_rate is not None else 'NA'} "
                f"base_unknown={base_unknown if base_unknown is not None else 'NA'} "
                f"optkua_unknown={optkua_unknown if optkua_unknown is not None else 'NA'}"
            )
    ...
    with relation_csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for rel, c in sorted(rel_stats.items()):
            qn = c["queries"]
            fn = c["num_exact_feasible"]
            fallback_n = c["num_fallback"]
            writer.writerow([
                rel,
                qn,
                (fn / qn) if qn > 0 else None,
                (fn / qn) if qn > 0 else None,
                (fallback_n / qn) if qn > 0 else None,
                (c["base_viol_sum"] / qn) if qn > 0 else None,
                (c["base_adm_sum"] / qn) if qn > 0 else None,
                (c["base_cov_sum"] / qn) if qn > 0 else None,
                (c["base_unknown_sum"] / qn) if qn > 0 else None,
                (c["optkua_viol_sum"] / qn) if qn > 0 else None,
                (c["optkua_adm_sum"] / qn) if qn > 0 else None,
                (c["optkua_cov_sum"] / qn) if qn > 0 else None,
                (c["optkua_unknown_sum"] / qn) if qn > 0 else None,
                (c["pres_sum"] / qn) if qn > 0 else None,
                (c["shift_sum"] / qn) if qn > 0 else None,
    ...
    summary = {
        ...
        "experiment_name": "unknown_aware_global",
        ...
        "semantic_policy": {
            ...
            "use_disjoint": args.use_disjoint,
            "beta": args.beta,
            "unknown_aware_energy": (
                "0 for admissible, beta for unknown, max(1, num_violations) for checkable violations"
            ),
            "deployment_policy": (
                "Use exact OptK-UA when enough admissible candidates exist in top-M; "
                "otherwise fall back to admissible-first / unknown-second / violating-last ranking."
            ),
        },
        ...
        "queries": {
            "total_queries": n,
            "num_exact_feasible": agg["num_exact_feasible"],
            "num_fallback": agg["num_fallback"],
            "exact_feasible_rate": (agg["num_exact_feasible"] / n) if n > 0 else None,
            "used_exact_optkua_rate": (agg["num_exact_feasible"] / n) if n > 0 else None,
            "used_fallback_rate": (agg["num_fallback"] / n) if n > 0 else None,
        },
        ...
        "base": {
            ...
            "cov_at_k": (agg["base_cov_sum"] / n) if n > 0 else None,
            "unknown_at_k": (agg["base_unknown_sum"] / n) if n > 0 else None,
        },

        "optkua": {
            "viol_at_k": (agg["optkua_viol_sum"] / n) if n > 0 else None,
            "adm_at_k": (agg["optkua_adm_sum"] / n) if n > 0 else None,
            "cov_at_k": (agg["optkua_cov_sum"] / n) if n > 0 else None,
            "unknown_at_k": (agg["optkua_unknown_sum"] / n) if n > 0 else None,
    ...
    logger.log(
        f"[DONE] exact_feasible={summary['queries']['exact_feasible_rate'] if summary['queries']['exact_feasible_rate'] is not None else 'NA'} "
        f"base_unknown={summary['base']['unknown_at_k'] if summary['base']['unknown_at_k'] is not None else 'NA'} "
        f"optkua_unknown={summary['optkua']['unknown_at_k'] if summary['optkua']['unknown_at_k'] is not None else 'NA'} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="EXP-05: Unknown-aware global evaluation over frozen KGC rankings."
    )
